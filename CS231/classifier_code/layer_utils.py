...
def affine_relu_dropout_forward(x, w, b,dp_params):
    """Convenience layer that performs an affine transform followed by a ReLU.
    ...
    a, fc_cache = affine_forward(x, w, b)
    out, relu_cache = relu_forward(a)
    out, drop_cache = dropout_forward(out,dp_params)
...
def affine_relu_dropout_backward(dout, cache):
    ...
    dp = dropout_backward(dout,drop_cache)
...
def affine_batchnorm_relu_forward(x, w, b,gamma,beta,bn_params):
    ...
    Returns a tuple of:
    - out: output from the batchnorm of shape (N, D)
    - cache: A tuple of values needed in the backward pass
    
    """
    a, fc_cache = affine_forward(x, w, b)
    out, batchnorm_cache = batchnorm_forward(a, gamma, beta, bn_params)
...
def affine_layernorm_relu_forward(x, w, b,gamma,beta,bn_params):
    ...
    """
    a, fc_cache = affine_forward(x, w, b)
    out, layernorm_cache = layernorm_forward(a, gamma, beta, bn_params)
    out, relu_cache = relu_forward(out)
...
def affine_layernorm_relu_backward(dout, cache):
    ...
    da = relu_backward(dout, relu_cache)
    da, dgamma, dbeta = layernorm_backward(da,layernorm_cache)
    dx, dw, db = affine_backward(da, fc_cache)
