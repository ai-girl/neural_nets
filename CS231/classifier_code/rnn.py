...
class CaptioningRNN:
    ...
    def __init__(
        ...
        self.idx_to_word = {i: w for w, i in word_to_idx.items()} # understand this line of code 
        self.params = {}
    ...
    def sample(self, features, max_length=30):
        ...
        T = max_length
        N,D = features.shape
        captions = np.zeros([N,T]).astype(int) 
        h0, cache1 = affine_forward(features,W_proj,b_proj) 
        h = h0
        c = np.zeros(h.shape)
        ...
        for i in range(0,T):
          ...
          if self.cell_type =='rnn':
            ...
            h, cache1 = rnn_step_forward(x,h,Wx,Wh,b)

          else:

            h,c,cache1 = lstm_step_forward(x, h, c, Wx, Wh, b)
 
          scores,cache2 = affine_forward(h,W_vocab,b_vocab)

          captions[:,i] = np.argmax(scores,axis=1)

          x = word_embedding_forward(captions[:,i],W_embed)[0]
